...
#
# Updated for mod_perl 2 by Doug Bell
...
#
# Wherever possible, I have used the response constants from Apache2::Constants,
...
sub copy_file
{
    ...
    {
        return Apache2::Const::HTTP_PRECONDITION_FAILED; # Precondition Failed?
    }
    ...
    if(!$fh)
    {
        return Apache2::Const::HTTP_CONFLICT; # huh?
    }

    print $fh $file;
    $handler->close_write($fh);

    return Apache2::Const::HTTP_CREATED; # Created.
}
...
sub mkcol
{
    ...
    if($content)
    {
        return Apache2::Const::HTTP_UNSUPPORTED_MEDIA_TYPE; # huh?
    }
    elsif(!$handler->test('e', $path))
    {
        ...
        {
            return Apache2::Const::HTTP_CONFLICT; # What?
        }
        else
        {
            return Apache2::Const::HTTP_CREATED; # Created.
...
sub move
{
    ...
    if($copy_result != 201)
    {
        if($copy_result == 412)
        {
            return Apache2::Const::HTTP_PRECONDITION_FAILED;
        }
        elsif($copy_result == Apache2::Const::HTTP_NO_CONTENT) # Directory already existed
        {
            return Apache2::Const::HTTP_FORBIDDEN;
        }
        else
